#include "hndl_pointcloud.h"

#include "pcl_header.pb.h"
#include "pcl_pfield.pb.h"
//#include "pcl.pb.h" // ROS/HRL
//#include "foxglove/PointCloud.pb.h" // foxglove
...
{
	std::vector<pcl_pfield::PointField> fieldsToParse;
...
template<typename T> void setPointCloud(POINTCLOUD_CLASS* pointcloud,
	std::vector<std::string> pointFieldNames, std::vector<T> points)
{
...
template<typename T> void set_fox_PointCloud(foxglove::PointCloud* pointcloud,
	std::vector<std::string> pointFieldNames, std::vector<T> pts)
{
	char fieldSizeUniform = sizeof(T);
	char pointFieldStride = pointFieldNames.size() * fieldSizeUniform; // (byte) --> #fields (s. below)

	// move to handler >>>
	for (int i = 0; i < pointFieldNames.size(); i++)
	{
		foxglove::PackedElementField* field = pointcloud->add_fields();
		field->set_name(pointFieldNames[i]);
		field->set_offset(fieldSizeUniform * i);
		field->set_type(foxglove::PackedElementField_NumericType_FLOAT32);
	}

	int pointFieldCount = pts.size(); // e.g. 1000 pts, 8 fields --> 8000
	int dataSize = pointFieldCount * fieldSizeUniform;
	char* bytes = (char*)malloc(dataSize);

	if (bytes != 0)
	{
		memcpy(bytes, pts.data(), dataSize);
	}
	...
	pointcloud->set_data(bytes, dataSize);
	pointcloud->set_point_stride(pointFieldStride); // # bytes/pt

	free(bytes);
	// <<< move to handler
}
template void set_fox_PointCloud(foxglove::PointCloud* pointcloud,
	std::vector<std::string> pointFieldNames, std::vector<float> pts);
